 ...
 *
 * Requires DATABASE_URL. Safe to re-run (checksum skip).
 */
import "dotenv/config";
import { createHash } from "crypto";
import { getPrisma, isDatabaseConfigured } from "../server/kb/db/client";
import { CROPS } from "../server/kb/corpus/crops";
import { DISEASES } from "../server/kb/corpus/diseases";
import { DISEASES_EXTRA } from "../server/kb/corpus/diseases-extra";
import { PESTS } from "../server/kb/corpus/pests";
import { PESTS_EXTRA } from "../server/kb/corpus/pests-extra";
import { ACTIVE_INGREDIENTS } from "../server/kb/corpus/products";
import { buildCorpusChunks } from "../server/kb/corpus/build";
      ...
      JSON.stringify(
        {
          ...
          hint: "Set DATABASE_URL to a Postgres instance with vector/pg_trgm/unaccent, then: npx prisma migrate deploy && npm run kb:migrate-corpus",
        },
        null,
        2
      )
